...
def create_grid_image(inputs, outputs, depth, max_num_images_to_save=3):
    ...
    im_vis_input = make_grid(inputs, nrow=batch_size // 2, padding=2, normalize=True, scale_each=True)
    #grid_input = im_vis_input.numpy().transpose(1, 2, 0)
    
    output_rgb = depthTensor2rgbTensor(outputs)
    depth_rgb = depthTensor2rgbTensor(depth)

    dephs= torch.cat((output_rgb, depth_rgb), 2)

    im_vis_depth = make_grid(dephs, nrow=batch_size // 2, padding=2, normalize=True, scale_each=True)
    #grid_depths = im_vis_depth.numpy().transpose(1, 2, 0)
    #plt.imshow(grid_depths)
    #plt.show()
        
    return im_vis_input, im_vis_depth